...
axs[1,0].axhline(y=500,color="red",linewidth=2)
axs[1,0].plot(v_line)
...
axs[1,1].set_ylabel('Pixel Intensity [a.u.]')
...
while (pixel < filt.size - 1):
    if (filt[pixel] == True):
        ...
        if (filt[pixel+1] == False):
            widths.append(pixel_count)
            pixel_count = 0
    pixel = pixel + 1
print(f"Raw pixel widths for each bar: {widths}")
avg_width = np.round(np.average(widths))
...
print(f'Magnfication: {magnification}')
